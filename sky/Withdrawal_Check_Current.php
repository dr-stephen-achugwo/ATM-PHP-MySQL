<?php 
  ...
  if($amt==0)	{
    ...
    echo 'alert("Please Enter a valid amount!"); window.location="Withdraw_Cash_Current.php"';
    echo '</script>';	
  }
  ...
  if($amt%100==0) {	
    if (mysqli_num_rows($result)>0) {
      while($row=mysqli_fetch_assoc($result)) {
        if ($row["current_balance"]>=$amt) {
          ...
          echo '<script language="javascript">';
          echo 'alert(" You have Insufficient funds in your account"); window.location="Withdraw_Cash_Current.php"';
          echo '</script>';
        }
      }
    }
    ...
    echo '<script language="javascript">';
    echo 'alert("Please Enter the amount in multiples of 100");window.location="Withdraw_Cash_Current.php"';
    echo '</script>';
  }